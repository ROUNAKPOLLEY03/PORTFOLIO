...
const Education = () => {
  return (
    ...
    >
      {/* Section Headers */}
      <motion.h4 className="text-center mb-2 text-lg font-ovo" variants={fadeInUp}>
      ...
      >
        {educationData.map(({ icon, title, description, link }, ind) => (
          <motion.div
            ...
            whileHover={{ scale: 1.07, rotate: 2, boxShadow: "0px 10px 20px rgba(0,0,0,0.2)" }}
          >
            ...
            <h3 className="text-lg my-4 text-gray-700 dark:text-white text-center font-semibold">{title}</h3>
            <p className="text-sm text-gray-600 leading-5 dark:text-white/80 text-center">{description}</p>
            <a href={link} className="flex justify-center items-center gap-2 text-sm mt-5 text-blue-500 font-medium">
              Read more{" "}
